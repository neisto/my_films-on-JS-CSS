 ...
 * @returns {{filmName: string, filmType: string, filmGenre: string, status: Boolean}} Объект дела
 */
function makeTodoItemFromData(data) {
  ...
  const todoItem = {
    ...
    filmName: data.filmName && String(data.filmName),
    status: Boolean(data.status),
    filmType : data.filmType && String(data.filmType),
    filmGenre : data.filmGenre && String(data.filmGenre)
  };
  console.log(todoItem);
  // проверяем, все ли данные корректные и заполняем объект ошибок, которые нужно отдать клиенту
  if (!todoItem.filmName) errors.push({ field: 'filmName', message: 'Не указан filmName' });
  if (!todoItem.filmType) errors.push({ field: 'filmType', message: 'Не указан filmType' });
  if (!todoItem.filmGenre) errors.push({ field: 'filmGenre', message: 'Не указан filmGenre' });
 ...
 * @returns {{filmName: string, filmType: string, filmGenre: string, status: Boolean}[]} Массив дел
 */
 ...
 * @param {{filmName: string, filmType: string, filmGenre: string, status: Boolean}} data - Объект с изменяемыми данными
 * @throws {TodoApiError} Дело с таким ID не найдено (statusCode 404)
 * @throws {TodoApiError} Некорректные данные в аргументе (statusCode 422)
 * @returns {{filmName: string, filmType: string, filmGenre: string, status: Boolean}} Объект дела
 */
